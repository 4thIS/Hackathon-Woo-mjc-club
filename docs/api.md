...
전체 피드용. **동아리를 가리지 않고 최신순.** (`/clubs/{id}/posts`가 오래된 순인 것과 반대다 —
거기는 한 동아리의 역사를 따라가는 화면이고, 여기는 지금 무슨 일이 있는지 보는 화면이다)
...
```json
{ "items": [ /* PostSummary[] */ ], "total": 42, "has_more": true }
```

- **검색 대상은 제목 · 본문 · 동아리명 · 태그**다. 대소문자를 가리지 않는다.
  태그는 JSON 배열을 문자열로 보고 찾으므로 정확한 태그 일치가 아니라 부분 일치다.
- 비로그인·미소속: 공개 글만. 소속 동아리의 비공개 글은 섞여 나오고, **남의 동아리 비공개 글은
  검색어가 맞아도 나오지 않는다.** 관리자는 전부 본다. (기획서 §6.2)
- `total`은 **현재 검색 조건 기준**이다. 전체 개수로 고정하면 '더 보기'가 끝나지 않는다.
...
### `DELETE /api/admin/clubs/{id}` — `관리자`

동아리를 지운다. **되돌릴 수 없다.** `204`

보통은 `PATCH` 의 `status: "보관"` 으로 충분하다 — 기록이 남고 되돌릴 수 있다. 삭제는 잘못 만들어진 동아리를 치우는 용도다.

**함께 지우는 것**: 소속(`club_members`) · 가입 신청 · 탈퇴 요청 · 가입폼 · 활동 글과 그 글의 좋아요.

**남기는 것**: 개설 신청 이력(`club_applications`). 누가 언제 신청했는지는 동아리와 별개의 기록이다.

화면은 삭제 전에 **동아리 이름을 직접 입력**받는다(관리자 화면 규칙).
...
계정을 지운다. **되돌릴 수 없다.** `204`

본인 탈퇴(`DELETE /me`)와 **같은 규칙**을 쓴다(`auth.purge_user`). 소속·가입/탈퇴/개설 신청·좋아요·인증 토큰은 지우고, **활동 글은 남긴다** — 기록은 동아리의 자산이라 작성자만 비우고 화면에는 `탈퇴한 회원` 으로 뜬다.

지우고 나면 되돌릴 수 없으므로 세 경우를 미리 막는다.

| 에러 | code | 왜 |
|---|---|---|
| 404 | `NOT_FOUND` | |
| 409 | `CANNOT_DELETE_SELF` | 실수로 자기 로그인 수단을 잃는다. 본인은 내 정보에서 탈퇴한다 |
| 409 | `LEADER_CANNOT_LEAVE` | 동아리장이 사라지면 그 동아리 운영이 멈춘다. 먼저 교체한다 |
| 409 | `LAST_ADMIN` | 관리자가 0명이 되면 아무도 관리 화면에 들어갈 수 없다 |

화면은 삭제 전에 **학번을 직접 입력**받는다.
...
### `POST /api/ai/recommend` — `인증`

성향 설문으로 어울리는 동아리 **TOP 3** 을 추천한다. **결과를 저장하지 않는다** — 호출한 사람에게 한 번 돌려주고 끝이다(DB 에 쓰지 않는다).
...
{
  "items": [
    { "club_id": 3, "name": "고리사진부", "category": "학술·전공",
      "recruit_status": "모집중", "reason": "…" }
  ],
  "traits": ["몸을 움직이는 활동을 좋아함", "…"]   // 설문을 사람이 읽는 문장으로
}
```

후보는 **활동중인 동아리 전부**다(모집중으로 좁히지 않는다 — 후보가 너무 적어진다). 각 동아리의 소개·분야·모집상태와 **최신 공개 활동 2건**(제목·태그·발췌)을 모아 한 번 호출한다. 모집중을 우선하라는 지시는 프롬프트가 맡는다.

응답의 `club_id` 는 **서버가 다시 검증한다.** 후보 목록에 없는 id 는 버린다 — 지어낸 동아리가 화면에 오르지 않게 하기 위해서다.

| 에러 | code |
|---|---|
| 400 | `INVALID_INPUT` — `answers` 가 A/B 문자열이 아님 |
| 403 | `AI_KEY_NOT_REGISTERED` · `EMAIL_NOT_VERIFIED` |
| 409 | `NO_CLUBS` — 추천할 동아리가 없음 |
| 429 | `AI_QUOTA_EXCEEDED` |
| 502 | `AI_GATEWAY_ERROR` |
